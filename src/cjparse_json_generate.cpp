                                                ...
                                                bool generate_pretty_json)
{
  ...
  if (std::holds_alternative<cjparse::json_object> (JSON))
    ...
    {
      new_value
          = cjparse::cjparse_json_value (std::get<cjparse::json_array> (JSON));
    }
    ...
    {
      new_value
          = cjparse::cjparse_json_value (std::get<cjparse::json_null> (JSON));
    }
...
cjparse_json_generator::print_json_number (cjparse::json_number &num)
{
  std::visit ([this] (auto &val)
                { JSON_string = JSON_string + std::to_string (val); }, num);
}
