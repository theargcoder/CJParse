...
cjparse::return_the_value (std::vector<std::string> object_name_vector_in)
{
  ...
  while (it != object_name_vector_in.end ())
    {
      return_the_value_internal (*it, full_json, full_json, return_empty);
      it++;
    }
  if (!return_empty)
    value_to_return = full_json;
  ...
  if (std::holds_alternative<json_array> (temp_value))
    {
      ...
      while (iter != value_array.end ())
        {
          return_the_value_internal (object_name, iter->value, value_to_return,
                                     return_empty);
          iter++;
        }
    }
...
cjparse::check_if_type (std::vector<std::string> object_name_vector_in)
{
  ...
  while (it != object_name_vector_in.end ())
    {
      check_if_type_internal<T> (*it, full_json, full_json, bool_to_return);
      it++;
    }
  return bool_to_return;
}
  ...
  if (std::holds_alternative<json_array> (temp_value))
    {
      ...
      while (iter != value_array.end ())
        {
          check_if_type_internal<T> (name_to_check_if_type, iter->value,
                                     value_to_alter, bool_to_return);
          iter++;
        }
    }
}
